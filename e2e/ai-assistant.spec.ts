...
test.beforeEach(async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    const persist = {
      enabledModules: ['overview', 'income', 'envelopes', 'transactions', 'accessibility'],
      useCardLayout: false,
      uiMode: 'normal',
    };
...
test('AI Assistant opens and replies to a message', async ({ page }) => {
  ...
  await page.locator('[data-section-id="6"], [aria-label="Settings"]').first().click({ timeout: 5000 });
  ...
  const cacheSwitch = page.getByTestId('module-cacheAssistant').getByRole('switch', { name: /Cache the AI Assistant/i });
  if ((await cacheSwitch.getAttribute('aria-checked')) === 'false') {
