...
test('add envelope then expense then see in Transactions', async ({ page }) => {
  ...
  const settingsSlice = page.locator('[data-layout="wheel"] [data-section-id="6"]').first();
  await settingsSlice.click({ force: true });
  ...
  const transactionsSwitch = transactionsRow.getByRole('switch', { name: /Transactions/i });
  const checked = await transactionsSwitch.getAttribute('aria-checked');
  if (checked !== 'true') {
