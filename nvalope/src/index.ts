 ...
 * - GET /api/entitlements: current user's entitlements (creates user + session cookie on first request).
	...
	async fetch(
		...
		if (url.pathname === '/api/entitlements' && request.method === 'GET') {
			...
			if (!userId) {
				...
				setCookie = await issueSessionCookie(userId, env);
			}
			const entitlements = await getEntitlementsForUser(env, userId);
			const res = jsonResponse(entitlements, 200, headers);
