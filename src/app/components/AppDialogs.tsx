...
export interface AppDialogsProps {
  ...
  onOfflineReadyOpenChange: (open: boolean) => void;
  ...
  // One-time updates notice (envelope decimals bug fix)
  showUpdatesNotice: boolean;
  onUpdatesNoticeOpenChange: (open: boolean) => void;
}
...
export function AppDialogs({
  ...
  showUpdatesNotice,
  onUpdatesNoticeOpenChange,
}: AppDialogsProps) {
  return (
    <>
      <AlertDialog open={showUpdatesNotice} onOpenChange={(open) => { if (!open) onUpdatesNoticeOpenChange(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Updates</AlertDialogTitle>
            <AlertDialogDescription>
              A bug was fixed that prevented users from entering decimal numbers in envelope amounts.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => onUpdatesNoticeOpenChange(false)}>OK</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      <BackupFolderPrompt
      ...
      <AlertDialog open={offlineReady} onOpenChange={(open) => { if (!open) onOfflineReadyOpenChange(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Ready to work offline</AlertDialogTitle>
            <AlertDialogDescription>
              The app is now cached and will work fully offline after the first load. You can close this and keep using Nvalope without an internet connection.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => onOfflineReadyOpenChange(false)}>OK</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
