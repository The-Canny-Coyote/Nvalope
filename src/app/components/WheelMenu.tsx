...
function WheelMenuComponent({ sections, enabledModules, showCacheAnimation = false, accessibilityMode: _accessibilityMode = 'standard', selectedSection: controlledSelected, onSelectedSectionChange, sectionContentRef, onUserAction, onOpenAssistant, expandContentOutside = false }: WheelMenuProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-0">
      ...
      >
        {/* Wheel sections: paths first (non-selected then selected so selected draws on top), then icons, then labels on top */}
        {[
          ...
          return (
            ...
            >
              ...
              <motion.path
                d={createSectionPath(index, hoveredSection === section.id, isSelected)}
                fill={isSelected ? section.color : hoveredSection === section.id ? `${section.color}cc` : `${section.color}99`}
                stroke="var(--primary)"
                strokeWidth={isSelected ? strokeBase + 2 : hoveredSection === section.id ? strokeBase + 1 : strokeBase}
        ...
        {/* Icons and hit targets: full <g> per section for accessibility and keyboard */}
        ...
        {isCacheEnabled && (
          <g
            ...
            role="button"
            tabIndex={0}
            aria-label="Open Cache the AI Assistant"
            style={{ cursor: onOpenAssistant ? 'pointer' : undefined }}
          >
            <title>Open Cache the AI Assistant</title>
            <motion.circle
              cx={centerX}
              cy={centerY}
              r="40"
        ...
        <AnimatePresence mode="wait">
          {selectedSectionData && (
            <motion.div
              ...
              transition={motionTransition}
              className="w-full max-w-[min(42rem,100%)] mt-1 min-w-0"
            >
              <Card className="bg-card border-primary/30 shadow-xl relative w-full min-w-0 overflow-x-auto">
