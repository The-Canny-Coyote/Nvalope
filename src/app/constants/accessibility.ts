...
export function clampLetterSpacing(value: number): number {
  return Math.min(LETTER_SPACING_MAX, Math.max(LETTER_SPACING_MIN, value));
}
...
export function clampWheelScale(value: number): number {
  return Math.min(WHEEL_SCALE_MAX, Math.max(WHEEL_SCALE_MIN, value));
}

export function clampCardBarScale(value: number): number {
  return Math.min(CARD_BAR_SCALE_MAX, Math.max(CARD_BAR_SCALE_MIN, value));
}

export function clampCardBarRows(value: number): number {
  return Math.min(CARD_BAR_ROWS_MAX, Math.max(CARD_BAR_ROWS_MIN, Math.round(value)));
}

export function clampCardBarColumns(value: number): number {
  return Math.min(CARD_BAR_COLUMNS_MAX, Math.max(CARD_BAR_COLUMNS_MIN, Math.round(value)));
}

export function clampCardsSectionWidth(value: number): number {
  return Math.min(CARDS_SECTION_WIDTH_MAX, Math.max(CARDS_SECTION_WIDTH_MIN, value));
}

export function clampScrollbarSize(value: number): number {
  return Math.min(SCROLLBAR_SIZE_MAX, Math.max(SCROLLBAR_SIZE_MIN, value));
}
