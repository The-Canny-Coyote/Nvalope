/**
 * Basic AI assistant: keyword-based replies. Used when Advanced AI cache is not enabled.
 * Optional performanceTier enables "Beefed Up" logic (e.g. burn rate) on medium/high-tier devices.
 * Uses keyword clusters (synonyms) for broader phrasing; stateless—no conversation context.
 */
...
export function getAssistantReply(
  ...
  if (containsAny(q, GREETING_KEYWORDS) && q.length < 80) {
    return "Hi! I'm Cache, your budget assistant. I can help with spending, what's left, income, envelopes, and how to add expenses or income. All data stays on your device.";
  }
